        ...
        <div className="crm-shell min-h-screen bg-surface text-ink font-sans flex">
            ...
            <aside className="w-56 shrink-0 bg-white border-r border-border flex flex-col">
                <div className="p-4 border-b border-border">
                    <Link to="/crm" className="flex items-center gap-2" data-testid="crm-logo">
                ...
                <nav className="p-2 flex flex-col gap-0.5 text-sm">
                    <RailLink to="/crm" icon={<LayoutGrid className="w-4 h-4" />} label="Dashboard" testid="crm-nav-dashboard" end />
                    <RailLink to="/crm/pipeline" icon={<KanbanSquare className="w-4 h-4" />} label="Pipeline" testid="crm-nav-pipeline" />
                    <RailLink to="/crm/passport-expiry" icon={<StampIcon className="w-4 h-4" />} label="Passport expiry" testid="crm-nav-expiry" />
                    <RailLink to="/crm/offline-case" icon={<PlusSquare className="w-4 h-4" />} label="New offline case" testid="crm-nav-offline" />
                    <RailLink to="/crm/reports" icon={<BarChart3 className="w-4 h-4" />} label="Reports" testid="crm-nav-reports" />
                    {user?.role === "admin" && (
                        <>
                            <div className="mt-4 px-2 text-[10px] uppercase font-mono tracking-widest text-ink-muted">Admin</div>
                            <RailLink to="/crm/products" icon={<Layers className="w-4 h-4" />} label="Visa products" testid="crm-nav-products" />
                            <RailLink to="/crm/consultants" icon={<Users2 className="w-4 h-4" />} label="Consultants" testid="crm-nav-consultants" />
                        </>
                    )}
                </nav>
