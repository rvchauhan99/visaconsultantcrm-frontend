...
import { Upload, FileText, Check, Loader2, User, Save, ScanLine, Archive } from "lucide-react";
...
export default function Apply() {
    ...
    if (!schema) return <div className="p-10 text-ink-muted">Loading…</div>;
    ...
    const travelerReady = requiredTravelerFields.every((k) => (traveler[k] || "").trim() !== "");

    const submit = async (outcome = "success") => {
        setSubmitting(true);
        try {
            const draftRes = await api.post("/cases", {
                visa_product_id: productId,
                traveler,
                field_values: fields,
                document_uploads: Object.entries(uploads).map(([doc_key, u]) => ({ doc_key, file_url: u.file_url, filename: u.filename })),
            });
            const checkout = await api.post("/cases/checkout", { draft_id: draftRes.data.draft_id, outcome });
        ...
        <div className="max-w-4xl mx-auto px-6 md:px-10 py-10">
            ...
            <div className="mb-8 flex items-center gap-3">
                ...
                <div>
                    ...
                    <div className="text-xs font-mono uppercase tracking-widest text-ink-muted">Processing {schema.processing_time_days} days · {INR.format(total)}</div>
                </div>
            </div>

            <div className="bg-white border border-border rounded-xl p-6 md:p-8">
                {step === 0 && <TravelerStep traveler={traveler} setTraveler={setTraveler} profiles={profiles} onPrefill={prefillFromProfile} saveAsProfile={saveAsProfile} setSaveAsProfile={setSaveAsProfile} />}
                ...
                <div className="mt-8 flex items-center justify-between pt-6 border-t border-border">
                    ...
                    {step < 4 && (
                        <button
                            ...
                            disabled={(step === 0 && !travelerReady) || (step === 1 && !allFieldsFilled) || (step === 2 && !allRequiredUploaded)}
                            className="text-sm px-6 py-2.5 rounded-full bg-navy text-white hover:bg-navy-hover disabled:opacity-40 disabled:cursor-not-allowed"
                        >
                            Continue →
                        </button>
                    )}
...
function TravelerStep({ traveler, setTraveler, profiles, onPrefill, saveAsProfile, setSaveAsProfile }) {
    ...
    return (
        <div>
            ...
            <div className="grid md:grid-cols-2 gap-5">
                ...
                <Field label="Passport expiry" required><input type="date" data-testid="traveler-passport-expiry" className={inp} value={traveler.passport_expiry_date || ""} onChange={(e) => upd("passport_expiry_date", e.target.value)} /></Field>
...
function DocUploader({ doc, value, onUpload }) {
    ...
    useEffect(() => {
        // Only fetch vault for vault-eligible doc keys
        if (["passport_scan", "photo"].includes(doc.doc_key)) {
        ...
        <div className="p-4 bg-surface border border-border rounded-xl" data-testid={`upload-${doc.doc_key}`}>
            <div className="flex items-center gap-4">
                <div className="flex-1">
                    ...
                    <p className="text-[10px] font-mono uppercase text-ink-muted mt-1">{doc.formats.join(", ")} · max {doc.max_size_mb}MB</p>
