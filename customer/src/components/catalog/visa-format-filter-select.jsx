...
import { VISA_FORMAT_BUCKETS } from "@/lib/visa-format-filter"
import { cn } from "@/lib/utils"

/**
 * Visa issuance Type filter — attachment-style rows: selected brand dot + Label · count.
 */
export function VisaFormatFilterSelect({
  value = "any",
  onChange,
  counts = {},
  className = "",
  ...
  const selected = useMemo(
    () => VISA_FORMAT_BUCKETS.find((b) => b.value === value) || VISA_FORMAT_BUCKETS[0],
    [value],
  )
    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <button
          ...
          aria-expanded={open}
          aria-label={`Visa type: ${selected.label}`}
          className={cn(
...
export default VisaFormatFilterSelect
