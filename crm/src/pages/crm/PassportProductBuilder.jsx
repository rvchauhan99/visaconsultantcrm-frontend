...
function productIdOf(schema) {
  return schema.passport_product_id;
}

function apiBase(schema) {
  return `/admin/passport-products/${productIdOf(schema)}`;
}

export default function PassportProductBuilder() {
  const { productId } = useParams();
  const [schema, setSchema] = useState(null);

  const load = useCallback(() => {
    return api.get(`/admin/passport-products/${productId}`).then((r) => setSchema(r.data));
  }, [productId]);
  ...
  const publish = async () => {
    try {
      await api.patch(`${apiBase(schema)}/publish`);
  ...
  const unpublish = async () => {
    await api.patch(`${apiBase(schema)}/unpublish`);
  ...
  return (
    <div className="p-6">
      <Link to="/passport-products" className="text-xs text-ink-muted hover:text-ink font-mono mb-3 inline-block">← Passport products</Link>
      ...
      <div className="flex items-baseline justify-between mb-4 flex-wrap gap-3">
        <div>
          ...
          <div className="text-xs font-mono uppercase tracking-widest text-ink-muted">
            {schema.country_flag} {schema.country_name} · {schema.passport_service_type}
          </div>
        </div>
      ...
      <div className="grid lg:grid-cols-[1fr_360px] gap-4">
        <Tabs defaultValue="basic">
          <TabsList className="bg-surface-card border border-border rounded-sm h-auto p-0.5">
            {["basic", "documents", "fields", "pricing"].map((t) => (
              <TabsTrigger key={t} value={t} className="text-xs uppercase font-mono tracking-widest px-3 py-1.5 rounded-sm data-[state=active]:bg-navy data-[state=active]:text-white" data-testid={`ppb-tab-${t}`}>{t}</TabsTrigger>
            ))}
          </TabsList>
        ...
        <aside className="bg-surface border border-border rounded-sm p-3">
          <div className="text-[10px] uppercase font-mono tracking-widest text-ink-muted mb-2">Preview</div>
...
function BasicTab({ schema, reload }) {
  const [form, setForm] = useState({
    title: schema.title,
    passport_service_type: schema.passport_service_type || "fresh",
  ...
  const save = async () => {
    try {
      await api.patch(apiBase(schema), form);
      toast.success("Saved");
      reload();
    } catch (e) {
  ...
  return (
    <div className="bg-surface-card border border-border rounded-sm p-4 mt-3 space-y-3" data-testid="basic-tab">
      <div className="grid grid-cols-2 gap-3">
        <F label="Title">
          <input className={inp} value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} data-testid="basic-title" />
        </F>
        <F label="Passport service type">
          <SearchableSelect
            clearable={false}
            value={form.passport_service_type}
            onChange={(v) => setForm({ ...form, passport_service_type: v || "fresh" })}
...
function DocsTab({ schema, reload }) {
  ...
  const add = async () => {
    ...
    try {
      await api.post(`${base}/documents`, n);
  ...
  const del = async (doc) => {
    ...
    try {
      await api.delete(`${base}/documents/${doc.id}`);
  ...
  const move = async (idx, dir) => {
    ...
    try {
      await api.post(`${base}/documents/reorder`, [
    ...
    <div className="bg-surface-card border border-border rounded-sm p-4 mt-3" data-testid="docs-tab">
            ...
            <tr key={d.doc_key} className="border-b border-border last:border-0">
              <td className="px-2 py-1.5">
                <div className="flex items-center gap-0.5">
                  <button onClick={() => move(idx, -1)} disabled={idx === 0} className="p-0.5 text-ink-muted hover:text-navy disabled:opacity-30"><ArrowUp className="w-3 h-3" /></button>
                  <button onClick={() => move(idx, 1)} disabled={idx === schema.documents.length - 1} className="p-0.5 text-ink-muted hover:text-navy disabled:opacity-30"><ArrowDown className="w-3 h-3" /></button>
                </div>
              </td>
              ...
              <td className="px-2 py-1.5"><SampleFileCell doc={d} base={base} reload={reload} /></td>
              <td className="px-2 py-1.5 text-right">
                <button onClick={() => del(d)} className="text-xs text-ink-muted hover:text-ink inline-flex items-center gap-1">
      ...
      <div className="border-t border-border pt-3">
        ...
        <div className="grid grid-cols-3 gap-2 text-sm">
          <MasterSelect kind="document" value={n.doc_key || null} onChange={pickMaster} testId="new-doc-key" />
          <input className={inp} placeholder="Display name" value={n.doc_name} onChange={(e) => setN({ ...n, doc_name: e.target.value })} disabled={!n.doc_key} />
          <label className="text-xs flex items-center gap-1.5"><input type="checkbox" checked={n.required} onChange={(e) => setN({ ...n, required: e.target.checked })} disabled={!n.doc_key} /> Required</label>
        ...
        <button onClick={add} disabled={!n.doc_key} className="mt-2 text-sm px-3 py-1.5 bg-navy text-white rounded-sm hover:bg-navy-hover disabled:opacity-40">Add document</button>
      </div>
    </div>
  );
}

function SampleFileCell({ doc, base, reload }) {
  ...
  const save = async (newUrl) => {
    try {
      await api.patch(`${base}/documents/${doc.id}`, { doc_key: doc.doc_key, sample_file_url: newUrl || null });
  ...
  return (
    <div className="flex items-center gap-1.5">
      ...
      <button onClick={() => setEditing(true)} className="text-ink-muted hover:text-navy" title="Set sample URL">
...
function FieldsTab({ schema, reload }) {
  ...
  const add = async () => {
    ...
    try {
      await api.post(`${base}/fields`, body);
  ...
  const del = async (f) => {
    ...
    try {
      await api.delete(`${base}/fields/${f.id}`);
      toast.success("Removed");
      reload();
    } catch (e) {
      toast.error(e.response?.data?.detail || "Failed");
  ...
  return (
    <div className="bg-surface-card border border-border rounded-sm p-4 mt-3" data-testid="fields-tab">
        ...
        <tbody>
          {schema.fields.map((f) => (
            <tr key={f.field_key} className="border-b border-border last:border-0">
              ...
              <td className="px-2 py-1.5 text-right">
                <button onClick={() => del(f)} className="text-xs text-ink-muted hover:text-ink inline-flex items-center gap-1">
                  <Trash2 className="w-3 h-3" /> Remove
                </button>
              </td>
            </tr>
      ...
      <div className="border-t border-border pt-3">
        ...
        <div className="grid grid-cols-3 gap-2 text-sm">
          <MasterSelect kind="field" value={n.field_key || null} onChange={pickMaster} testId="new-field-key" />
          <input className={inp} placeholder="Label" value={n.label} onChange={(e) => setN({ ...n, label: e.target.value })} disabled={!n.field_key} />
          <SearchableSelect
            clearable={false}
            value={n.field_type}
          ...
          {n.field_type === "dropdown" && <input className={inp + " col-span-2"} placeholder="Options (comma separated)" value={n.options} onChange={(e) => setN({ ...n, options: e.target.value })} disabled={!n.field_key} />}
        </div>
        <button onClick={add} disabled={!n.field_key} className="mt-2 text-sm px-3 py-1.5 bg-navy text-white rounded-sm hover:bg-navy-hover disabled:opacity-40">Add field</button>
      </div>
    </div>
  );
}
...
function PricingTab({ schema, reload }) {
  ...
  const save = async () => {
    try {
      await api.post(`${apiBase(schema)}/fees`, { govt_fee: Number(gov), service_fee: Number(srv), currency: "INR" });
  ...
  return (
    <div className="bg-surface-card border border-border rounded-sm p-4 mt-3 space-y-3" data-testid="pricing-tab">
      ...
      <div className="grid grid-cols-2 gap-3">
        <F label={FEE_LABELS.govt}><input type="number" className={inp} value={gov} onChange={(e) => setGov(e.target.value)} /></F>
        <F label={FEE_LABELS.service}><input type="number" className={inp} value={srv} onChange={(e) => setSrv(e.target.value)} /></F>
      </div>
      <div className="flex justify-between items-center flex-wrap gap-2">
        ...
        <button onClick={save} className="text-sm px-3 py-1.5 bg-navy text-white rounded-sm hover:bg-navy-hover">Save new pricing</button>
      </div>
    </div>
  );
}

function PreviewCard({ schema }) {
  ...
  return (
    <div className="bg-surface-card border border-border rounded-lg overflow-hidden">
      ...
      <div className="p-3">
        ...
        <div className="text-[10px] font-mono uppercase text-ink-muted mt-2">{schema.processing_time_days}d process · {schema.passport_service_type}</div>
        <div className="pt-3 mt-3 border-t border-border flex items-end justify-between">
          ...
          <Stamp tone="gold" size="sm">Passport</Stamp>
        </div>
